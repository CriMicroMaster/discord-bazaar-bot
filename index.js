...
async function updateMemberCountChannel(guild) {
  ...
  try {
    ...
    if (channel.type !== 0) {
      console.warn(`Channel is not a text channel. Type: ${channel.type}`);
      return;
    }
    ...
    if (channel.name === newName) {
      console.log("Channel name already up to date.");
      return;
    }
    ...
    console.error("❌ Error updating member count channel:", err);
  }
}
...
client.on("voiceStateUpdate", async (oldState, newState) => {
  ...
  if (!oldState.channelId && newState.channelId) {
    ...
    if (newState.channelId !== afkChannelId) {
      ...
      console.log(`[${currentTime}] ${user.username} joined a channel.`);
    } else {
  ...
  if (oldState.channelId && newState.channelId) {
    ...
    if (newState.channelId !== afkChannelId) {
      ...
      const user = newState.member.user; // Get the user object from the newState
      const currentTime = new Date().toLocaleTimeString();
      console.log(`[${currentTime}] ${user.username} switched to another channel.`);
    } else {
      voiceActivity.delete(userId);
    }
  }
  ...
  if (oldState.channelId && !newState.channelId) {
    ...
    const currentTime = new Date().toLocaleTimeString();
    console.log(`[${currentTime}] ${user.username} left a channel.`);
  }
});
...
// Function to add reactions to the role message
